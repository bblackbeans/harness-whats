...
import { ArrowDownLeft, ArrowUpRight, CircleDot, RefreshCw } from "lucide-react";
...
export default function LogsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const logs = await listOpsLogs(150, tenantFilter || undefined);
              ...
              {loading && events.length === 0 ? (
                ...
                events.map((event, index) => (
                  <tr key={`${event.ts}-${event.message_id}-${event.status}-${index}`}>
                    <td className="py-2 pr-3 whitespace-nowrap text-gray-500">{formatTime(event.ts)}</td>
